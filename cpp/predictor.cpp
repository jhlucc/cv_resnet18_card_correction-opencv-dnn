...
cv_resnet18_card_correction::cv_resnet18_card_correction(const string model_path)
{
	this->model = readNet(model_path);
	this->outlayer_names = this->model.getUnconnectedOutLayersNames();
}

Mat cv_resnet18_card_correction::preprocess(const Mat& srcimg)
{
	Mat img;
...
myDict cv_resnet18_card_correction::infer(const Mat& srcimg)
{
...
Mat cv_resnet18_card_correction::crop_image(const Mat& img, const vector<Point2f>& position)
{
...
myDict cv_resnet18_card_correction::postprocess(const std::vector<cv::Mat>& output, const cv::Mat& image)
{
